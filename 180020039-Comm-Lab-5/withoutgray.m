...
%XXXX 16 - QAM Without Gray labelling :  BER vs SNR Plot   XXXXX
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
close all;
%-------------------Data to be sent--------------------------------%
N = 10000; %Number of bits in the input signal
x= randi([0,1],1,N);%randomly generate bits of 0,1
M=16; %number of symbols
bits = log2(M); %Number of bits per symbols

%---------Modulation/ QAM Constellation : Without Gray labelling-----------%
%16-QAM constellation has 16 symbols mapped to different phases and
%amplitudes 
%initialise tx as empty array for storing non- gray labelling constellation
yy=[]; % transmitted signal initialisation
...
for i=1:bits:length(x)
    ...
    yy=[yy y]; %appending the transmitted signal
end
%scatterplot(yy)
...
n=(1/sqrt(2))*[randn(1,length(yy))+1j*randn(1,length(yy))]; %generating gaussian complex noise
...
xlabel('Eb/N0(dB)');
ylabel('BER');
title('16 QAM : Without Gray labelling')
