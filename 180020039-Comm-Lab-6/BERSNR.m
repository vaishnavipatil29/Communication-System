...
% for given values of SNR values for DQPSK constellation with gray labelling 
...
% Inputs: x - generated constellation
...
%dbpsk detection
%loop is ran till length(x_cap)-1 as the final detected signal should have
%1 less bit than the bpsk detected signal
op=[];
for ii=1:length(x_cap)-1
        if x_cap(ii)==x_cap(ii+1);  %if two consecutive bits are same the detcted bit is 1     
            op(ii)=1;
        else
...
ber_sim=sum(op~=x)/N;    %ber_sim for a aprticular snr
