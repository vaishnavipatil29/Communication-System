% This function returns a randomly generated DQPSK constellation with gray labelling 
...
function [b gray wogray]= qpskconstellation(N)
%generate N random binary bits of 0s and 1s 
b = randi([0,1],1,N);
%-------Differential Encoding----%
d=randi([0 1],1); %Initial bit : randomly choose between 0 or 1

x=[]; %initialise differential encoded modified transmitted bits
for i=1:length(b)
    x=[x d];   %append the delayed bit to the modified transmitted bits
    d=not(xor(d,b(i))); %computing the next bit of b
end
x=[x d]; %appending the last bit to the modified transmitted bits
